...
function initMap() {
	...
	map = new google.maps.Map(mapDiv, {
		center: { lat: $aktiv.data("lat"), lng: $aktiv.data("lng") },
		scrollwheel: false,
		zoom: 17
	});
...
$().ready(function () {
	if ($(".fooldal-carousel").length) {
		var carouselStep = function ($item) {
			$item.fadeIn(200, function () {
				$nextItem = $item.next().length ? $item.next() : $item.siblings(".fooldal-carousel-item").first();
				setTimeout(function () {
					$item.fadeOut(1000, function () {
						carouselStep($nextItem);
					});
				}, 1000);
			})
		};

		carouselStep($(".fooldal-carousel-item").eq(0));
	}

	$(".fooldal").on("click", ".header-fomenupont, .fooldal-carousel-btn", function (e) {
		e.preventDefault();
	...
	$(".header").affix({
		offset: {
			top: function () {
				return $(".fooldal-carousel").height() - 5
			}
		}
	});
	...
	$(".header-fomenu-toggle").on("click", function (e) {
		e.preventDefault();
		$(".header-fomenu").toggleClass("open");
	});

	$(".contact").on("click", function () {
		var location = { lat: $(this).data("lat"), lng: $(this).data("lng") };

		$(".contact").removeClass("aktiv");
		$(this).addClass("aktiv");
		map.panTo(location);
	});

	if ($(window).width() > 768) {
		$('.who-we-are').parallaxie({
			'speed': 0.1,
			'size': 'auto',
			'offset': 0
		});

		$('.references').parallaxie({
			'speed': 0.1,
			'size': 'auto',
			'offset': 0
		});

		$('.parallaxie-section').parallaxie({
			'speed': 0.3,
			'offset': 1
		});
	}
